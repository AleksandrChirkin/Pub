...
exports.index = function(req, res) {
    ...
    async.parallel({
        ...
        res.render('index', { title: 'КРУГЛОСУТОЧНЫЙ ПАБ', error: err, data: results });
    });
};
...
exports.table_list = function(req, res) {
    ...
    async.parallel({
		...
		for (let i=0; i<results.orders.length; i++) {
			if (results.orders[i].order_time.getTime() + results.orders[i].duration*3600000 < Date.now()) { //столик заказан (и его бронь не закончена)
        ...
        res.render('/', { title: 'Столики', error: err, data: results });
    });
};
